...
class BluetoothConnection:
    ...
    async def _request_data(self):
        ...
        while self._connected:
            try:
                ...
                temp = await self._client.read_gatt_char(_TEMPERATURE_HANDLE)
                pressure = await self._client.read_gatt_char(_PRESSURE_HANDLE)
                humidity = await self._client.read_gatt_char(_HUMIDITY_HANDLE)
    ...
    def _encode_pressure(self, raw_pressure):
        ...
        return ("{:.2f}".format(float_pressure), "hPa")
